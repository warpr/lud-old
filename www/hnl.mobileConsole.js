...
var mobileConsole = (function() {
    ...
    var options = {
        ...
        originalConsole = {
            ...
            groupEnd:
                typeof console.groupEnd === 'function' ? console.groupEnd.bind(console) : null,
    ...
    function getMaxHeight() {
        return valBetween(
            Math.floor(
                (window.innerHeight || document.documentElement.clientHeight) * options.ratio
            ),
            55,
            300
        );
    ...
    var setLineStyle = (function() {
            ...
            return function(element, type, msg) {
                if (status.initialized) {
                    color =
                        msg === 'undefined' || msg === htmlToString(messages.empty)
                            ? { color: '#808080' }
                            : msg === htmlToString(messages.clear)
                                ...
                                : lineStyles(type) !== undefined
                                    ...
                                    : lineStyles.log.text;
                    dot =
                        lineStyles(type) !== undefined ? lineStyles(type).dot : lineStyles.log.dot;
    ...
    function initConsole() {
        ...
        function formatStackTrace(trace, origtrace) {
            ...
            var stackTraceOrig =
                trace !== undefined && trace[4] !== undefined ? trace[4].stack : undefined;
        ...
        function objectToTable(table, object) {
            var i;
            for (i in object) {
                var tdLeft = elements.td.cloneNode(false),
                    tr = elements.tr.cloneNode(false);
        ...
        function newConsole() {
            try {
                ...
                var method = arguments[0],
                    className,
                    message =
                        arguments[1].newMessage !== undefined ? arguments[1].newMessage : undefined,
                    stackTrace =
                        arguments[1].newStackTrace !== undefined
                ...
                if (isRepeat(message, method) && method.indexOf('time') === -1) {
                    ...
                    if (
                        ...
                        if (className.indexOf('HTML') !== -1 && className !== 'HTMLDocument') {
                            message = htmlToString(
                                message.outerHTML.match(/<(.*?)>/g)[0] +
                                    '...' +
                                    message.outerHTML.match(/<(.*?)>/g).pop()
                            ); //gets first and last tag, adds '...' in middle. e.g. <div>...</div>
                        } else {
    ...
    function initCommandLine() {
        ...
        function setBinds() {
                ...
                if (e.target === elements.consoleinput) {
                    if (e.key === 'Enter' || e.keyCode === 13) {
                        ...
                        if (status.acActive) {
                            ...
                            elements.consoleinput.value =
                                hist[history.input.commandIdx] === undefined
                                    ? ''
            ...
            document.onkeyup = function(e) {
                if (
                    e.target === elements.consoleinput &&
